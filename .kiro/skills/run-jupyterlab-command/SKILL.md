...
```bash
just start --no-browser --expose-app-in-browser
```

If the server is already running without this flag, use the `restart-jupyter-server` skill to stop it, then restart with the flag.
